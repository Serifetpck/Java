...
public class Soru3 {
    public static void main(String[] args) {
      ...
      int kactane=4;
        Arrays.sort(input);
        int enbuyuk=input[input.length-1];

        for (int i = 1; i <=kactane-1 ; i++) {


        for (int j = input[input.length-(i+1)] ; j >input[input.length-(i+2)] ; j--) {

            enbuyuk1+=" "+input[input.length-(i+1)];

        }
        }
        System.out.println(enbuyuk+" "+enbuyuk1);
                                           //HOCANIN ÇÖZDÜĞÜ//
